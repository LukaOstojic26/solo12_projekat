...
class TaskJointSpline(TaskBase):
    ...
    def following_spline(self, t, q, qv, sensors):   ###### bilo je bez sensors
        self.ref_position = self.trajectory(t)
        self.ref_velocity = self.trajectory(t, 1)
        self.control = self.joint_controller.compute_control(self.ref_position, self.ref_velocity, q, qv, sensors)   ##### bilo je bez sensors
        return t >= self.transition_end

    def compute_control(self, t, q, qv, sensors):
        self.following_spline(t, q, qv, sensors)   ###### bilo je:      self.tick(t, q, qv)
        return self.control
